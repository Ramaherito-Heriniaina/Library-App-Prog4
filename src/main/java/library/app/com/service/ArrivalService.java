...
public class ArrivalService {
  ...
  public List<Arrival> getAll(int page, int pageSize) {
    return repository.findAll(PageRequest.of(page, pageSize)).stream().map(Arrival::from).toList();
  }
  ...
  public List<Arrival> getByBookId(Long bookId) {
    return repository.findByBookId(bookId).stream().map(Arrival::from).toList();
  }
}
