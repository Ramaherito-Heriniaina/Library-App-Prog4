...
public class Book {
    ...
    public static Book from(JBook j) {
        ...
        return Book.builder()
            ...
            .authorIds(j.getAuthors() != null
                ? j.getAuthors().stream().map(JAuthor::getId).toList() : List.of())
            .categoryIds(j.getCategories() != null
                ? j.getCategories().stream().map(JCategory::getId).toList() : List.of())
            .build();
    }
}
