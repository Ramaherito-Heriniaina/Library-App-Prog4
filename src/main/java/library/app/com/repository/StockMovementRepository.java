...
public interface StockMovementRepository extends JpaRepository<JStockMovement, Long> {
  ...
  /**
   * Quantité pour un livre + une édition spécifique (format GF ou PF) = SUM(IN) - SUM(OUT) filtrés
   * sur le format de la BookCopy
   */
  @Query(
      """
      SELECT COALESCE(SUM(
  ...
  Integer computeStockQuantityByFormat(
      @Param("bookId") Long bookId, @Param("format") String format);
}
